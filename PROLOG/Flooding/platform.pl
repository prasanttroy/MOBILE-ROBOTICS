...
% start_tartarus/2 starts the agent platform at a port which can be specified by the user. It creates a platform agent, writes some output for
...
:-dynamic start_tartarus/1.

start_tartarus(P):-
		start_tartarus(localhost,P).
        
start_tartarus(Ip,Port):-
        start_db,
...
start_tartarus(_Ip,_Port):-
        print_message(error,'start_tartarus(Ip,Port) failed, Please enter valid IP address and unused Port number'),fail.
...
%% Gives current platform IP address and port number
...
worker_Tartarus_IITG(Queue,Port):-                                      %% Read connection one by one from queue +Queue and process their messages as input
   ...
   (agent_GUID(Agent_name,_,(_,Port))->
        ...
        ;
        ((Agent_name = platform; Agent_name= server)->
                %catch(thread_create(execute_msg_Tartarus_IITG(Functor,Out,From),Thread,[]),Error,writeln(Error));
                execute_msg_Tartarus_IITG(Functor,Out,From);
                format(Out,'~q.~n',[fail]),
                flush_output(Out))),
...
agent_post(_Platform,(_Ip_receiver,_Port_receiver),_FunctionList):-
        print_message(error,'post_agent(platform,(Ip_receiver,Port_receiver),FunctionList) failed'),fail,!.
...
create_static_agent(Agent_name,(Platform_Ip,Port),Handler):-
        platform_state(on),
...
:-dynamic create_mobile_agent/2.
:-dynamic create_mobile_agent/3.

create_mobile_agent(GUID,Handler):- agent_create(GUID,Handler).  % -- to avoid confusion and for easy recalling, a new name is defined -- %

create_mobile_agent(GUID,(Ip,Port),Handler):- agent_create(GUID,(Ip,Port),Handler).
...
agent_create(_GUID2,(_Ip,_Port),_Handler):-
        print_message(warning,'agent_create(GUID2,(Ip,Port),Handler) failed').
...
execute_agent(Agent_name,(Ip,Port),Handler):- agent_execute(Agent_name,(Ip,Port),Handler). % -- adding a simple name --%
...
agent_execute(Agent_name,(Ip,Port),Handler):-                                           %% create agent +Agent_name +Handler +Ip +Port
        (agent_GUID(Agent_name,Handler,(Ip,Port))->
				...
				ttyflush,!;
                print_message(error,'Agent name provided does not exist'),!).

agent_execute(_Agent_name,(_Ip,_Port),_Handler):-
        print_message(warning,'execute_agent(Agent_name,(Ip,Port),Handler) failed!'),!.


execute_agent(Agent_name,(Ip,Port),Handler,Start_function):- agent_execute(Agent_name,(Ip,Port),Handler,Start_function). % -- adding a simple name --%
...
agent_clone(GUID1,(Send_Ip,Send_Port),GUID2):-
        ((atom(GUID1),integer(Send_Port))->nothing;abort),
        gensym(GUID1,GUID2),
...
agent_clone(GUID1,(_Ip,_Port),_):-
        print_message(error,'Clone agent Failed for: '),write(GUID1),nl.
...
agent_move(GUID,(Ip_other_end,Port_other_end)):-
        ...
        (agent_post(platform,(Ip_other_end,Port_other_end),[handler,platform,(Ip,Port),send(GUID,Handler,PayloadList,AgentCode,ListOfTokens2,TT)])->
...
% parameters. In case a platform has a security token set(via set_token/1), the incoming agent's token list is checked. If it contains the
...
handler( platform, (Ip_sender,Port_sender), send(GUID,Handler,PayloadList,[Code|CodeList],ListOfTokens,_BackTime)):-                            %//
        ...
        write('Agent bearing '),write(GUID),
        write(' has arrived to this platform. '),nl,
        get_platform_details(Platform_Ip,Platform_Port),
        (thread_peek_message(clean_agent_queue,agent_guid(GUID))->
                cleanup_wait(GUID)
                %handler(platform,(Platform_Ip,Platform_Port),clean_agent(GUID))
...
% and therefore kept hidden from the user of this platform{Means that if he creates a assert_code_Tartarus_IITG/1 of his own, my predicate here won't
